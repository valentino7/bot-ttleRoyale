
oak='CAADBAADewYAAvSBbQm2It_LTkk9awI'

role='Da scrivere'

commands= '/bottle_royale: apertura gioco'

TOKEN = '628533657:AAGJXWNwcqARguCKXDn0M61tVJQ5OPIDWx4'

attack=['CgADBAADggADr9OEUhX2l731sjrLAg','CgADBAADqQADaD2EUlCEvPoYnlarAg','CgADBAADowADZx-EUhYbWZk6yIQZAg','CgADBAAD6gADk4p8UspbE7k1XL-YAg']
troll=['CgADBAADhgADEZ2NUhPU8ldF86LqAg','CgADBAADxwADY5-NUjKlv-FIe6gkAg','CgADBAADcAAD6QuMUq-3Ji0-Fkg6Ag','CgADBAAD0gADihiMUuJ5MS7pwDyEAg','CgADBAADZgADuCWNUijI684KgHt2Ag','CgADBAAD1gADjkmMUmVz-EI-8ivJAg','CgADBAADhQADB0CNUg_EDznR61qJAg','CgADBAADrwADaJmNUuGl_6PwksbBAg','CgADBAAD5wADjN-NUnfI6PCwPPkyAg','CgADBAAD7AADEzuEUnsaalihypOxAg','CgADBAADZQADkVOMUqVp6pTtROKdAg']

kboardAttack='⚔️ ATTACCA ⚔️'
kboardDefense='🛡 DIFENDI 🛡'
kboardEvocation='💫 EVOCAZIONE 💫'
kboardEffect='💫 USA EVOCAZIONE 💫'
kboardPrayer='🙏 PREGHIERA 🙏'
kboardEscape='🏃\u200d♂️ Esci dal gioco 🏃\u200d♂️'

root_gif_path='./gif/'
att_path='att/'
def_path='def/'
troll_def_path='troll_def/'
troll_att_path='troll_att/'
troll_ev_path='troll_ev/'
ev_path='ev/'
extension='.gif'
back='🔙 Indietro 🔙'

attackTroll=[-3,-7,-10]
evocationTroll=['perditurno','sceglichifarperdereturno']
evocation=['attx2','defx2','seibersaglio','nontiattaccano','doppioturno']
keyboardEvocation=['2️⃣✖️ Attx2','2️⃣✖️ Defx2','❌ Sei bersaglio','⛔️ Non ti attaccano','2️⃣✖️ Doppio turno']
#il valore 1 significa: no difesa
defenseTroll=[-5,-10,1]